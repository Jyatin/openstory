...
import { llmtrCompatibleApi, llmtrTextCostFromUsage } from '@/models/llmtr';
import {
  isRegionBlockedLlmError,
  regionFallbackModel,
} from '@/models/region-policy';
import { aiDebugLogger } from '@/platform/server/ai/ai-debug-logger';
import {
  createAdapter,
...
export type LLMRequestParams<T = unknown> = {
   ...
   * (from `@/platform/server/ai/ai-debug-logger`) to see full payloads in local Workerd
